...
import (
	...
	"github.com/jrife/ptarmigan/storage/raft"
...
type KVRaftStore struct {
	...
	snapshotter    raft.SnapshotterCb
	Store          kv.SubStore
	name           []byte
}
...
func (raftStore *KVRaftStore) Entries(lo, hi, maxSize uint64) ([]raftpb.Entry, error) {
	...
	transaction, err := raftStore.Store.Begin(false)
...
func (raftStore *KVRaftStore) OnSnapshot(cb raft.SnapshotterCb) {
